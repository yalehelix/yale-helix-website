// Google Apps Script for handling startup file uploads to Google Drive
// Updated to support chunked uploads and better error handling

// Replace this with your specific Google Drive folder ID
const FOLDER_ID = '1uFGlOK6KgwnAcjlZFJjd9xpdmT_Pq4el';
...
function handleChunkedUpload(data) {
  try {
    ...
    // Get the main folder directly (no subfolder creation)
    const folder = getMainFolder();
...
function getMainFolder() {
  try {
    // Get the main folder by ID directly
    const mainFolder = DriveApp.getFolderById(FOLDER_ID);
    console.log('Using main startup folder:', FOLDER_ID);
    return mainFolder;
...
function doGet(e) {
  ...
  return ContentService
    .createTextOutput(JSON.stringify({
      message: 'Startup Google Apps Script is running',
      timestamp: new Date().toISOString(),
      version: '2.0',
      features: ['single-upload', 'chunked-upload', 'batch-upload', 'direct-folder-upload'],
...
function testUpload() {
  console.log('Testing startup upload functionality...');
  
  // Test data
  const testData = {
    fileName: 'test-pitch-deck.pdf',
    fileType: 'application/pdf',
    fileData: Utilities.base64Encode('Hello, this is a test startup file!'),
    uploadMethod: 'single'
  };
