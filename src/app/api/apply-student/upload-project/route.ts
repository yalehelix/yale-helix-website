...
import { getSupabaseAdmin, PROJECT_FILE_BUCKET } from "@/lib/supabaseAdmin";

const ALLOWED_EXTENSIONS = [".pdf", ".doc", ".docx", ".png", ".jpg", ".jpeg", ".zip"];

// Issues a one-time signed URL the browser uses to upload a project file directly
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ bucket: PROJECT_FILE_BUCKET, path, token: data.token });
  } catch (err) {
